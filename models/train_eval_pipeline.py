...
def main():
    ...
    with tf.Session() as sess:
        model = BiLSTM(hidden_size=[FLAGS.hidden_size], embedding_size=embeddings.get_fastText_embedding, max_length=FLAGS.max_length, learning_rate=FLAGS.learning_rate)
        ...
        for epoch in xrange(FLAGS.epochs):
            # train_batch_generator = utils.generate_train_batch(FLAGS.batch_size)
            # for X_train, y_train in train_batch_generator:
            X_train, y_train = utils.generate_train_batch(FLAGS.batch_size)
            feeds_train = [
                model.loss,
                model.train_step,
                model.merged
            ...
            if (global_step + 1) % 101 == 0:
                ...
                X_val, y_val = utils.get_validation_data()
                feed_val = [
                    model.loss,
                    model.accuracy,
                    model.merged
